...
#define GCHECK_DO(exp,action)	{if(!(exp)){Logger::write("<%s,%s,%s(%d),%s(%d)>", #exp,__FUNCTION__,__FILE__,__LINE__,errno,strerror(errno));action;}}
...
#define GCHECKV_DO(exp,action,fmt,...)	{if(!(exp)){Logger::write("<%s,%s,%s(%d),%s(%d)>" fmt, #exp,__FUNCTION__,__FILE__,__LINE__,errno,strerror(errno),__VA_ARGS__);action;}}
